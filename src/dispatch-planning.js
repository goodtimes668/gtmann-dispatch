...
export function findDispatchConflicts(list){
  ...
  var active=list.filter(function(booking){
    return (booking.status==='approved'||booking.status==='in-progress')&&booking.date&&booking.time&&booking.assignedTo;
  });
  for(var i=0;i<active.length;i++){
    for(var j=i+1;j<active.length;j++){
      var a=active[i], b=active[j];
      if(a.date!==b.date||a.assignedTo!==b.assignedTo) continue;
