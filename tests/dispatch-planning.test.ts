...
describe("dispatch capacity conflicts", () => {
  ...
  it("leaves unassigned work for the ownership metric instead of inventing a dispatcher conflict", () => {
    const ids = findDispatchConflicts([
      { id: "a", status: "approved", date: "2026-08-23", time: "09:00", assignedTo: "", durationMinutes: 90 },
      { id: "b", status: "approved", date: "2026-08-23", time: "09:30", assignedTo: "", durationMinutes: 90 },
    ]);
    expect([...ids]).toEqual([]);
  });
});
